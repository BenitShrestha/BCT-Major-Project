"""
ROUGE Evaluation Script for Fine-tuned Nepali Summarization Model (CSV Input)
- Input CSV: title,summary,text
- Reference-length-conditioned generation
- Top-k + Top-p sampling
- Devanagari-only preprocessing
- ROUGE-1 / ROUGE-2 / ROUGE-L evaluation
- Saves:
    - CSV with detailed scores
    - JSON with reference and generated summaries
...
class EvalConfig:
    ...
    TEST_CSV: str = r"C:\Users\prash\Documents\AI\Major Project\FineTuning\nepali_XLSum_v2.0\Filtered\filtered_train.csv"
    OUTPUT_CSV: str = r"C:\Users\prash\Documents\AI\Major Project\FineTuning\finetuned-summarization\results\csv_rouge_results.csv"
    OUTPUT_JSON: str = r"C:\Users\prash\Documents\AI\Major Project\FineTuning\finetuned-summarization\results\rouge_scores.json"
...
def get_rouge_scorer():
    return rouge_scorer.RougeScorer(["rouge1","rouge2","rougeL"], tokenizer=NepaliTokenizer(), use_stemmer=False)
...
def evaluate():
    ...
    csv_out = open(cfg.OUTPUT_CSV,"w",newline="",encoding="utf-8")
    ...
    # Read CSV
    with open(cfg.TEST_CSV,"r",encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for idx,row in enumerate(tqdm(reader, desc="Evaluating")):
            ref_summary = preprocess_nepali(row["summary"])
            gen_summary = preprocess_nepali(generate_summary(model, sp, row["text"], row["summary"], cfg, device))
    ...
    aggregate_scores = {m:{"p":avg(agg[m]["p"]),"r":avg(agg[m]["r"]),"f":avg(agg[m]["f"])} for m in agg}

    # Save JSON
    with open(cfg.OUTPUT_JSON,"w",encoding="utf-8") as f:
        json.dump({"results":ref_gen_json, "aggregate_rouge":aggregate_scores}, f, ensure_ascii=False, indent=2)

    # Save CSV aggregates
    writer.writerow([])
    writer.writerow(["# AGGREGATE ROUGE SCORES"])
    for m in agg:
        writer.writerow([m.upper(), aggregate_scores[m]["p"], aggregate_scores[m]["r"], aggregate_scores[m]["f"]])
    ...
    print("="*50)
    for m in agg:
        print(f"{m.upper():7s} | F1: {aggregate_scores[m]['f']*100:.2f}")
    print("="*50)
